                                        ...
                                        <div class="dropdown-menu">
                                            ...
                                            @can('user_export')
                                                <a target="_blank" class="dropdown-item"
                                                    href="{{ route('user_export', ['type' => 'key', ...request()->query()]) }}">
                                                    {{ __('匯出參數版') }}
                                                </a>
                                                <a target="_blank" class="dropdown-item"
                                                    href="{{ route('user_export', ['type' => 'value', ...request()->query()]) }}">
                                                    {{ __('匯出文字版') }}
                                                </a>
                                            @endcan
                                        </div>
                                ...
                                <div class="btn-group">
                                    @can('user_create')
                                        <a class="btn btn-primary"
                                            href="{{ route('user_update', ['id' => 0]) }}?{{ request()->getQueryString() }}">
                                            {{ __('新增') }} </a>
                                    @endcan
                                </div>
                            </div>
                                                        ...
                                                        <td class="border-0 text-end" data-column="operate">
                                                            <div class="btn-group">
                                                                ...
                                                                <ul class="dropdown-menu">
                                                                    @can('user_update')
                                                                        <li><a class="dropdown-item" href="{{ route("user_update",["id"=>$Item->id])}}?{{ request()->getQueryString() }}">{{ __('編輯') }}</a>
                                                                        </li>
                                                                    @endcan
                                                                    @can('user_delete')
                                                                        <li><button class="dropdown-item" type="button"
                                                                                onclick="postForm('{{route("user_del")}}?{{ request()->getQueryString() }}',{
                                                                        'id_array[]':{{ $Item->id }},
                                                                        _token:'{{ csrf_token() }}'
                                                                        })">{{ __('刪除') }}
                                                                            </button></li>
...
@section('Modal')
    ...
    <div class="slideFunc-box" id="listSetting">
        <div class="slideFunc-content">
            ...
            <form action="{{ route('user_saveListColumn') }}" method="POST">
                @csrf
                <div class="slideFunc-body px-3 py-3">
                    <div class="row">
                        <div class="col-12">
...
@section('BodyJavascript')
    <script>
        ...
        $("#btnDeleteBatch").on("click", function() {
            ...
            //送出
            postForm('{{route("user_del")}}?{{ request()->getQueryString() }}', postArray)
        });
