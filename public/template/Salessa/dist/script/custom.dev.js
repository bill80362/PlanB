...
(function ($) {
  "use strict"; // metisMenu

  $("#sidebar_menu").metisMenu(); // metisMenu
  ...
  $('.notification_clicker').on('click', function (e) {
    e.preventDefault();
    console.log($(this));
    $(this).parent('li').siblings('li').find('.Menu_NOtification_Wrap').removeClass('active').end().end().end().siblings('.Menu_NOtification_Wrap').toggleClass('active');
  });
  $(document).click(function (event) {
    if (!$(event.target).closest(".notification_clicker, .Menu_NOtification_Wrap").length) {
  ...
  var alertAry = document.getElementsByClassName('alert');
  console.log(alertAry);
  alertAry.forEach(function (ele, i) {
    ele.onload = setTimeout(function () {
